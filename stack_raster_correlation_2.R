...
sp <- readOGR("shp/Ptr Validare/fishnet_label.shp")
spdf <- as.data.frame(sp)
spdf <-spdf[, rev(colnames(spdf))]
cord <- spdf[1:2]
cord <- cord[,c(2,1)]
#######
...
#plot(ndvi1.xyz)
#### precipitation compozit
prec1 <- readRDS("tab/prec.rds")
head(prec1)
prec1.xyz  <- rasterFromXYZ(prec1, crs = "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0" )
#plot(prec1.xyz)
### stack la cele doua rastere
########incepe corelatia pentru fiecare compozit_.Momentan fara decalaj intre layere.
